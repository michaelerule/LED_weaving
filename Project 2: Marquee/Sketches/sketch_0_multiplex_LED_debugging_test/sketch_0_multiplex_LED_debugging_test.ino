...
// We can control NLINES*(NLINES-1) LEDs using NLINES pins. For now, we'll
// define the grid in terms of the charlielexing layout between pins, rather 
// than the actual physical arrangement of LEDs. NLINES should be odd.
#define NLINES 11
#define NROWS (NLINES+1)
...
inline void displayToGrid(int row, int col, int *i, int *j) {
  //row = (row-col+NROWS)%NROWS;
  //col = (col-row+NCOLS)%NCOLS;
  if (row<col) row++;
  *i = col;//lineToPin[col];
  *j = row;//lineToPin[row];
}
